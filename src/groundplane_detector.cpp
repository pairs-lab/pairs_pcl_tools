#include <pairs_pcl_tools/groundplane_detector.h>
...
namespace pairs_pcl_tools
{
  ...
  GroundplaneDetector::groundplane_detection_config_t::groundplane_detection_config_t(pairs_lib::ParamLoader& pl, const std::string& param_prefix)
  {
    loadParams(pl, param_prefix);
  }

  void GroundplaneDetector::groundplane_detection_config_t::loadParams(pairs_lib::ParamLoader& pl, const std::string& param_prefix)
  {
  ...
  void GroundplaneDetector::initialize(ros::NodeHandle& nh, const std::shared_ptr<pairs_lib::Transformer>& tfr, const groundplane_detection_config_t& cfg)
  {
    m_cfg = cfg;
    m_tfr = tfr;
    ...
    if (m_cfg.range_use)
    {
      ...
      pairs_lib::construct_object(m_sh_range, shopts, "rangefinder_in");
    }
  ...
  {
    auto transformer = std::make_shared<pairs_lib::Transformer>(NODE_NAME);
