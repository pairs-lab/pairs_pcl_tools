...
void PCLPublishCloudFileToNetwork::onInit() {
  ...
  pairs_lib::ParamLoader param_loader(nh, "PCLPublishCloudFileToNetwork");
  ...
  if (_load_colors) {
    ...
    cloud_loaded = pairs_pcl_tools::loadCloud(file_cloud, cloud_rgb, true);

  } else {

    const PC::Ptr cloud_xyz = boost::make_shared<PC>();
    cloud_loaded            = pairs_pcl_tools::loadCloud(file_cloud, cloud_xyz, true);

    if (cloud_loaded) {
      cloud_rgb = pairs_pcl_tools::visualization::colorizeCloud(cloud_xyz);
    }
  }
...
PLUGINLIB_EXPORT_CLASS(pairs_pcl_tools::PCLPublishCloudFileToNetwork, nodelet::Nodelet);
