...
void publishCloud(const ros::Publisher &pub, const pcl::PointCloud<T> &cloud) {
  if (pub.getNumSubscribers() > 0) {
    try {
      ...
      ROS_ERROR("[pairs_pcl_tools::publishCloud]: Exception caught during publishing on topic: %s", pub.getTopic().c_str());
    }
  }
}
/*//}*/
...
void publishCloudMsg(const ros::Publisher &pub, const sensor_msgs::PointCloud2::ConstPtr &cloud_msg) {
  if (pub.getNumSubscribers() > 0) {
    try {
      ...
      ROS_ERROR("[pairs_pcl_tools::publishCloudMsg]: Exception caught during publishing on topic: %s", pub.getTopic().c_str());
    }
  }
}
/*//}*/
...
void printEigenMatrix(const Eigen::Matrix4f &mat, const std::string &prefix) {
  ...
  pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(mat.block<3, 3>(0, 0).cast<double>());
