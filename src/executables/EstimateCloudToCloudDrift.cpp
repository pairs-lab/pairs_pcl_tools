...
void printHelp() {
  ...
  ROS_ERROR("Usage:");
  ROS_ERROR("   rosrun pairs_pcl_tools estimate_cloud_to_cloud_drift source.pcd target.pcd trajectory_in.txt trajectory_out.txt [...]");
  ...
  if (stat(filepath.c_str(), &buffer) == 0) {
    ...
    while (std::getline(infile, line)) {
      ...
      if (line_numbers.size() != 4 && line_numbers.size() != 8) {
        ...
        pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(quat);
...
void saveTrajectory(const std::string &filepath, const std::vector<TRAJECTORY_POINT> &data) {
  ...
  if (outfile.is_open()) {
    ...
    for (const auto &dato : data) {
      ...
      const pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(dato.transformed_pose.block<3, 3>(0, 0).cast<double>());
...
int main(int argc, char **argv) {
  ...
  // load pcds
  const auto ret_source = pairs_pcl_tools::loadPcXYZ(args.pcd_source);
