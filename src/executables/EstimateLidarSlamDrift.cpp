...
void printHelp() {
  ...
  ROS_ERROR("Usage:");
  ROS_ERROR(
      "   rosrun pairs_pcl_tools estimate_lidar_slam_drift rosbag.bag topic_cloud mapping_origin tf_map_in_target.mat target.pcd trajectory_odom_out.txt "
...
geometry_msgs::Pose eigenMatrixToPoseMsg(const Eigen::Matrix4f &mat) {
  ...
  const pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(mat.block<3, 3>(0, 0).cast<double>());
...
std::optional<Eigen::Matrix4d> loadStaticTransformFromFile(const std::string &filepath) {
  ...
  if (stat(filepath.c_str(), &buffer) == 0) {
    ...
    if (row == 4) {
      ...
      // Normalize rotation
      const pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(mat.block<3, 3>(0, 0));
...
bool parseArguments(int argc, char **argv, ARGUMENTS &args) {
  ...
  if (args.invert_tf_target_in_map_origin) {
    ...
    pairs_pcl_tools::printEigenMatrix(tf_target_in_map_origin_inverted.cast<float>(), "Target in map (result of inversion):");

    args.tf_target_in_map_origin = eigenMatrixToTransformMsg(tf_target_in_map_origin_inverted.cast<float>());

  } else {

    pairs_pcl_tools::printEigenMatrix(tf_target_in_map_origin.cast<float>(), "Target in map:");
...
void saveTrajectory(const std::string &filepath, const std::vector<TRAJECTORY_POINT> &data, const bool transformed_data) {
  ...
  if (outfile.is_open()) {
    ...
    for (const auto &dato : data) {
      ...
      const pairs_lib::AttitudeConverter atti = pairs_lib::AttitudeConverter(pose.block<3, 3>(0, 0).cast<double>());
...
std::vector<TRAJECTORY_POINT> estimateGroundTruthTrajectoryFromRosbag(const rosbag::Bag &bag, const PC::Ptr &pc_target, const ARGUMENTS &args) {
  ...
  const PC::Ptr cloud_target = pairs_pcl_tools::filters::applyVoxelGridFilter(pc_target, resolution);
  ...
  for (const rosbag::MessageInstance &msg : view) {
    ...
    if (perform_registration) {
      ...
      const PC::Ptr cloud_source = pairs_pcl_tools::filters::applyVoxelGridFilter(cloud_buffer.getCloudsAsOne(), resolution);
...
int main(int argc, char **argv) {
  ...
  // Load target cloud
  const auto ret_target = pairs_pcl_tools::loadPcXYZ(args.pcd_target);
  if (!ret_target) {
