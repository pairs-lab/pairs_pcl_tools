...
void PCLFiltration::onInit() {
  ...
  // Param loader
  _common_handlers->param_loader = std::make_shared<pairs_lib::ParamLoader>(nh, "PCLFilter");

  // Transformer
  const auto uav_name           = _common_handlers->param_loader->loadParam2<std::string>("uav_name");
  _common_handlers->transformer = std::make_shared<pairs_lib::Transformer>("PCLFilter");
  ...
  _common_handlers->scope_timer_logger   = std::make_shared<pairs_lib::ScopeTimerLogger>(time_logger_filepath, _common_handlers->scope_timer_enabled);
  ...
  if (_lidar3d_republish) {
    ...
    _sub_lidar3d                = pairs_lib::SubscribeHandler<sensor_msgs::PointCloud2>(shopts, "lidar3d_in", &PCLFiltration::lidar3dCallback, this);
...
void PCLFiltration::lidar3dCallback(const sensor_msgs::PointCloud2::ConstPtr msg) {
  ...
  diag_msg->sensor_type                                     = pairs_modules_msgs::PclToolsDiagnostics::SENSOR_TYPE_LIDAR_3D;
  ...
  const pairs_lib::ScopeTimer timer =
      pairs_lib::ScopeTimer("PCLFiltration::lidar3dCallback", _common_handlers->scope_timer_logger, _common_handlers->scope_timer_enabled);
...
void PCLFiltration::process_msg(typename boost::shared_ptr<PC>& inout_pc_ptr) {
  ...
  pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("PCLFiltration::process_msg", _common_handlers->scope_timer_logger, _common_handlers->scope_timer_enabled);
...
PLUGINLIB_EXPORT_CLASS(pairs_pcl_tools::PCLFiltration, nodelet::Nodelet);
