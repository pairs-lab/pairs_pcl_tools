...
TEST(PointcloudFilters, returnOriginal_VoxelGridFilter)
{
  ...
  auto cloud_filtered = pairs_pcl_tools::filters::applyVoxelGridFilter<PC_t>(logger, cloud, 0.0f);
...
TEST(PointcloudFilters, downsample_VoxelGridFilter)
{
  ...
  auto cloud_filtered = pairs_pcl_tools::filters::applyVoxelGridFilter<PC_t>(logger, cloud, 0.1f);
...
TEST(PointcloudFilters, applyRadiusOutlierFilter)
{
  ...
  auto cloud_filtered = pairs_pcl_tools::filters::applyRadiusOutlierFilter<PC_t>(logger, cloud, radius, min_neighbors, keep_organized);
...
TEST(PointcloudFilters, applyMinimumGridFilter)
{
  ...
  auto cloud_filtered = pairs_pcl_tools::filters::applyMinimumGridFilter<PC_t>(logger, cloud, resolution);
...
TEST(PointcloudFilters, applyCropBox)
{
  ...
  auto filtered_cloud = pairs_pcl_tools::filters::applyCropBox<PC_t>(logger, cloud, transform, min, max, keep_organized, set_negative);
