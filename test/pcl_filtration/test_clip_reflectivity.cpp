...
TEST(ReflectivityClipping, checkRangeField)
{
  auto [range_exists1, range_offset1] = pairs_pcl_tools::getFieldOffset<PointXYZR>("reflectivity");
  ASSERT_TRUE(range_exists1);

  auto [range_exists2, range_offset2] = pairs_pcl_tools::getFieldOffset<pcl::PointXYZI>("reflectivity");
...
{
  DummyLogger logger;
  pairs_pcl_tools::Lidar3DConfig cfg;
  ...
  pairs_pcl_tools::PCLFiltrationCore pcl_filtration_core(logger);
