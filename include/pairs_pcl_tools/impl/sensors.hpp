...
void SensorDepthCamera::initialize(const ros::NodeHandle& nh, const std::shared_ptr<CommonHandlers_t> common_handlers, const std::string& prefix,
  ...
  pairs_lib::SubscribeHandlerOptions shopts(_nh);
  shopts.node_name  = "SensorDepthCamera::CameraInfo::" + sensor_name;
  shopts.threadsafe = true;
  sh_camera_info    = pairs_lib::SubscribeHandler<sensor_msgs::CameraInfo>(shopts, depth_camera_info_in);
  pairs_lib::construct_object(sh_camera_info, shopts, depth_camera_info_in, ros::Duration(20.0), &SensorDepthCamera::process_camera_info_msg, this);

  if (mask_use) {
    sh_mask = pairs_lib::SubscribeHandler<sensor_msgs::Image>(shopts, mask_in);
    pairs_lib::construct_object(sh_mask, shopts, mask_in, ros::Duration(1.0), &SensorDepthCamera::process_mask_msg, this);
  }
...
void SensorDepthCamera::process_depth_msg(const sensor_msgs::Image::ConstPtr msg) {
  ...
  const pairs_lib::ScopeTimer timer       = pairs_lib::ScopeTimer(scope_label, _common_handlers->scope_timer_logger, _common_handlers->scope_timer_enabled);
  ...
  const pairs_modules_msgs::PclToolsDiagnostics::Ptr diag_msg = boost::make_shared<pairs_modules_msgs::PclToolsDiagnostics>();
  diag_msg->sensor_name                                     = sensor_name;
  diag_msg->stamp                                           = depth_msg->header.stamp;
  diag_msg->sensor_type                                     = pairs_modules_msgs::PclToolsDiagnostics::SENSOR_TYPE_DEPTH_CAMERA;
  ...
  if (voxel_grid_use) {
    cloud = pairs_pcl_tools::filters::applyVoxelGridFilter(cloud, voxel_grid_resolution);
  }
  if (radius_outlier_use) {
    cloud = pairs_pcl_tools::filters::applyRadiusOutlierFilter(cloud, radius_outlier_radius, radius_outlier_neighbors);
  }
  if (bilateral_use) {
    cloud = pairs_pcl_tools::filters::applyBilateralFilter(cloud, bilateral_sigma_S, bilateral_sigma_R);
  }
  if (minimum_grid_use) {
    cloud = pairs_pcl_tools::filters::applyMinimumGridFilter(cloud, minimum_grid_resolution);
  }
...
void SensorDepthCamera::process_camera_info_msg(const sensor_msgs::CameraInfo::ConstPtr msg) {
  ...
  if (valid) {
    ...
    if (intensity_use) {
      ...
      shopts.threadsafe = true;
      sh_depth          = pairs_lib::SubscribeHandler<sensor_msgs::Image>(shopts, depth_in);
      pairs_lib::construct_object(sh_depth, shopts, depth_in, ros::Duration(1.0), &SensorDepthCamera::process_depth_msg, this);
    }
  }
}
/*//}*/
