...
namespace pairs_pcl_tools
{
  ...
  struct Diagnostics_t
  {
    Diagnostics_t(ros::NodeHandle& nh) {
      _pub_diagnostics = nh.advertise<pairs_modules_msgs::PclToolsDiagnostics>("diagnostics_out", 10);
    }

    void publish(const pairs_modules_msgs::PclToolsDiagnosticsConstPtr& msg) const {
  ...
  struct CommonHandlers_t
  {
    std::shared_ptr<pairs_lib::ParamLoader> param_loader;
    std::shared_ptr<pairs_lib::Transformer> transformer;

    bool                                       scope_timer_enabled;
    std::shared_ptr<pairs_lib::ScopeTimerLogger> scope_timer_logger;
