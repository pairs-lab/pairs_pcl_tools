...
#include <pairs_pcl_tools/support.h>
#include <pairs_pcl_tools/pcl_filtration_core.h>
#include <pairs_pcl_tools/utils/ros_logger.h>
#include <pairs_pcl_tools/pcl_filtration_diagnostics.h>
...
namespace pairs_pcl_tools
{
  ...
  private:
    ...
    template <typename PC>
    void m_processPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg, pairs_modules_msgs::msg::PclToolsDiagnostics& diag_msg);
    ...
    template <typename PC>
    void m_logPointCloudStats(pairs_lib::ScopeTimer& timer, std::shared_ptr<PC>& inout_pc_ptr, const size_t height_before, const size_t width_before,
                              const size_t points_before);

  private:
    rclcpp::TimerBase::SharedPtr m_timer_init_;
    rclcpp::Node::SharedPtr m_node_;
    std::shared_ptr<pairs_lib::ParamLoader> m_param_loader_;
    std::shared_ptr<pairs_lib::Transformer> m_transformer_;
    std::shared_ptr<pairs_lib::DynparamMgr> m_dynparam_mgr_;
    std::shared_ptr<pairs_lib::ScopeTimerLogger> m_scope_timer_logger;
    ...
    pairs_lib::PublisherHandler<sensor_msgs::msg::PointCloud2> m_pub_lidar;
    pairs_lib::PublisherHandler<sensor_msgs::msg::PointCloud2> m_pub_lidar_over_max_range;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::PointCloud2> m_sub_lidar;
...
#include <pairs_pcl_tools/pcl_filtration_ros.tpp>
