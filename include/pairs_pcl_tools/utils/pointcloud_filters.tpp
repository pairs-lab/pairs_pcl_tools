namespace pairs_pcl_tools
{
  ...
  namespace filters
  {
    ...
    typename PC_t::Ptr applyVoxelGridFilter([[maybe_unused]] ILogger& logger, const typename PC_t::ConstPtr& cloud, const float resolution)
    {
      ...
      {
        logger.error(
            "[pairs_pcl_tools::filters::applyVoxelGridFilter] Resolution is less than or equal to zero, not applying voxel "
            "grid filter.");
    ...
    typename PC_t::Ptr applyBilateralFilter([[maybe_unused]] ILogger& logger, const typename PC_t::ConstPtr& cloud, const float sigma_S, const float sigma_R)
    {
      ...
      {
        logger.error("[pairs_pcl_tools::filters::applyBilateralFilter] Unorganized cloud given, not applying bilateral filter.");
        return cloud;
      }
