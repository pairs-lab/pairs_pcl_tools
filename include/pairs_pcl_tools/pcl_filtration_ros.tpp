namespace pairs_pcl_tools
{
  ...
  template <typename PC>
  void PCLFiltration::m_logPointCloudStats(pairs_lib::ScopeTimer& timer, std::shared_ptr<PC>& inout_pc_ptr, const size_t height_before, const size_t width_before,
  ...
  /*//{ m_processPointCloud() */
  template <typename PC>
  void PCLFiltration::m_processPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg, pairs_modules_msgs::msg::PclToolsDiagnostics& diag_msg)
  {
  ...
  void PCLFiltration::m_processMsg(std::shared_ptr<PC>& inout_pc_ptr)
  {
    ...
    pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer(m_node_, "PCLFiltration::process_msg", m_scope_timer_logger, m_scope_timer_enabled);
